// AaveMain.jsx — tab bar, admin panel behind Dashboard click
const { useState, useEffect } = React;

const AM = {
  bg: '#0a0a0e', panel: '#13141a', border: '#1e2028',
  text: '#e8e8f0', muted: '#6b6b82', teal: '#2ebac6',
  purple: '#b6509e', green: '#46bc8c', red: '#e84142',
...
const PRESETS = [
  ...
  { id: 'base-usdc-nogas', label: '15 USDC · Base',           sub: 'No ETH gas' },
  { id: 'base-usdc-gas',   label: '15 USDC · Base + gas',     sub: '+ 0.002 ETH' },
];

// ── Admin Panel (behind Dashboard click) ──────────────────────
function AdminPanel({ visible, onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300 }}
      ...
      <div style={{ background: AM.panel, border: '1px solid ' + AM.border, borderRadius: 18, padding: 24, width: 580, maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 12px 60px rgba(0,0,0,.8)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div>
            <div style={{ color: AM.text, fontWeight: 800, fontSize: 16 }}>Scenario Setup</div>
            <div style={{ color: AM.muted, fontSize: 11, marginTop: 2 }}>Facilitator controls</div>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: AM.muted, fontSize: 22, cursor: 'pointer' }}>×</button>
        </div>

        {/* Presets */}
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 11, color: AM.muted, fontWeight: 700, letterSpacing: '.05em', textTransform: 'uppercase', marginBottom: 10 }}>Presets</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {PRESETS.map(p => (
              <button key={p.id} onClick={() => AppState.applyPreset(p.id)} style={{
                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                background: AM.bg, border: '1px solid ' + AM.border, borderRadius: 10,
                padding: '10px 14px', cursor: 'pointer', textAlign: 'left',
              }}>
                <div>
                  <div style={{ color: AM.teal, fontSize: 13, fontWeight: 600 }}>{p.label}</div>
                  <div style={{ color: AM.muted, fontSize: 11 }}>{p.sub}</div>
                </div>
                <span style={{ color: AM.muted, fontSize: 16 }}>→</span>
              </button>
            ))}
          </div>
        </div>
        ...
        {/* Manual balances */}
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 11, color: AM.muted, fontWeight: 700, letterSpacing: '.05em', textTransform: 'uppercase', marginBottom: 10 }}>Manual Balances</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {[
              ...
              return (
                <div key={chain} style={{ background: AM.bg, borderRadius: 10, padding: 12 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: meta?.color || AM.text, marginBottom: 8 }}>{meta?.name}</div>
                  {tokens.map(tok => (
                    <div key={tok} style={{ marginBottom: 6 }}>
                      <div style={{ fontSize: 10, color: AM.muted, marginBottom: 3 }}>{tok}</div>
                      <input type="number" min="0" step={tok === 'USDC' ? '1' : '0.001'}
                        value={(state.balances[chain] && state.balances[chain][tok]) ?? 0}
                        onChange={e => AppState.setBalance(chain, tok, e.target.value)}
                        style={{ width: '100%', background: AM.panel, border: '1px solid ' + AM.border, borderRadius: 7, padding: '5px 8px', color: AM.text, fontSize: 12, outline: 'none', boxSizing: 'border-box' }} />
                    </div>
                  ))}
                </div>
              );
        ...
        {/* Chain speeds */}
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 11, color: AM.muted, fontWeight: 700, letterSpacing: '.05em', textTransform: 'uppercase', marginBottom: 10 }}>Transaction Speeds (seconds)</div>
          {CHAIN_SPEEDS_META.map(ch => (
            <div key={ch.id} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
              ...
              <div style={{ width: 32, textAlign: 'right', fontSize: 12, color: AM.text, fontWeight: 600 }}>{state.chainSpeeds?.[ch.id] ?? 5}s</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: AM.bg, borderRadius: 10, padding: 14, marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 10, color: AM.muted, marginBottom: 4 }}>Borrowed USDC</div>
              <input type="number" min="0"
                value={(state.aaveBorrowed && state.aaveBorrowed.USDC) || 0}
                onChange={e => AppState.aaveBorrow('USDC', (parseFloat(e.target.value)||0) - ((state.aaveBorrowed&&state.aaveBorrowed.USDC)||0))}
                style={{ width: '100%', background: AM.panel, border: '1px solid ' + AM.border, borderRadius: 7, padding: '5px 8px', color: AM.text, fontSize: 12, outline: 'none', boxSizing: 'border-box' }} />
            </div>
          </div>
        </div>

        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 14, borderTop: '1px solid ' + AM.border }}>
          <div style={{ fontSize: 11, color: AM.muted }}>
            Wallet: <span style={{ color: state.walletConnected ? AM.green : AM.red }}>{state.walletConnected ? '● Connected' : '● Disconnected'}</span>
          </div>
          <button onClick={() => AppState.reset()} style={{ background: 'rgba(232,65,66,.1)', border: '1px solid rgba(232,65,66,.3)', borderRadius: 8, padding: '6px 14px', color: AM.red, fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>Reset All</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ display: 'flex', alignItems: 'center', background: '#08080c', borderBottom: '1px solid #1a1a24', height: 38, flexShrink: 0 }}>
      {[
        ...
        <button key={tab.id} data-stani-target={tab.id === 'coinbase' ? 'coinbase-tab' : 'waave-tab'} onClick={() => onChange(tab.id)} style={{
          ...
          color: active === tab.id ? AM.text : AM.muted,
          borderRight: '1px solid #1a1a24',
          borderBottom: active === tab.id ? '2px solid ' + AM.teal : '2px solid transparent',
        }}>
